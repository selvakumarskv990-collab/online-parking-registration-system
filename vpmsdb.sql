...
(3, 'again', 'again@xyz.com', '123456789', 'Bicycles', 'sdf', '234567', '2019-10-05 08:09:45');
 ...
 CREATE TABLE `logtable` (
  ...
  `ParkingNumber` VARCHAR(20),
  `VehicleCategory` VARCHAR(50),
  `VehicleCompanyname` VARCHAR(50),
  `RegistrationNumber` VARCHAR(20),
  `OwnerName` VARCHAR(50),
  `OwnerContactNumber` BIGINT,
  `InTime` DATETIME,
  `OutTime` DATETIME,
  `ParkingCharge` VARCHAR(20),
  `Remark` VARCHAR(100),
  `Status` VARCHAR(20)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
DROP TABLE IF EXISTS `logtable_audit`;
CREATE TABLE `logtable_audit` LIKE `logtable`;
DELIMITER$$
...
begin
 ...
 VALUES(NEW.ID, NEW.ParkingNumber, NEW.VehicleCategory, NEW.VehicleCompanyname, NEW.RegistrationNumber, NEW.OwnerName, NEW.OwnerContactNumber, NEW.InTime, NEW.OutTime, NEW.ParkingCharge, NEW.Remark, NEW.Status);
end$$
DELIMITER ;


-- Create audit table for logging inserts


-- Create trigger to log inserts into logtable_audit
